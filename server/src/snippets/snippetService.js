...
async function create(snippetParams) {
  ...
  const { errors } = validate(snippetParams);

  if (errors) {
    throw new Error(errors.details[0].message);
  }
...
async function update(name, snippetParams) {
  ...
  if (
    snippet.name !== snippetParams.name &&
    (await Snippet.findOne({ name: snippetParams.name }))
  ) {
    throw new Error(
      'The snippet name "' + snippetParams.name + '" is already taken.'
    );
  }
